...
async function unFriend(obj, cb, socket, io) {
  ...
  if (user && requestUser) {
    ...
    await requestUser.friends.pull(d);
    ...
    await user.friends.pull(d1);

    await user.save();

    // Reload the user object from the database to get the updated values
  ...
  if (user && requestUser) {
    ...
    if (!alreadyFriend) {
      for (let i = 0; i < user.requestsSend.length; i++) {
        if (
          JSON.stringify(user.requestsSend[i]) ==
          JSON.stringify(requestUser._id)
        ) {
