...
const { updateTaskProgressData } = require("./taskProgressDataUpdator");
...
async function updatePlayerStats(obj, cb, socket, io) {
  ...
  if (user) {
    ...
    user.playerStat.vitality +=
      ...
      obj.stat.vitality.wastelandBottle * 0.01 +
      obj.stat.vitality.edenBottle * 0.01 +
      obj.stat.vitality.smallMediKit * 0.25 +
      obj.stat.vitality.largeMediKit * 0.5 +
      obj.stat.vitality.mediumMedikit * 0.35;
    ...
    for (let i = 0; i < obj.stat.gunHandling.length; i++) {
      ...
      if (obj.stat.gunHandling[i].gunName == "AK74") {
        user.playerStat.gunHandling.AK74 += obj.stat.gunHandling[i].shotFired * 0.05;

      }
      else if (obj.stat.gunHandling[i].gunName == "Tariq") {
        user.playerStat.gunHandling.Tariq += obj.stat.gunHandling[i].shotFired * 0.05;

      }
      else if (obj.stat.gunHandling[i].gunName == "PM84") {
        user.playerStat.gunHandling.PM84 += obj.stat.gunHandling[i].shotFired * 0.05;

      }
      else if (obj.stat.gunHandling[i].gunName == "Mossberg") {
        user.playerStat.gunHandling.Mossberg += obj.stat.gunHandling[i].shotFired * 0.05;

      }
      else if (obj.stat.gunHandling[i].gunName == "M24") {
        user.playerStat.gunHandling.M24 += obj.stat.gunHandling[i].shotFired * 0.05;

      }
      else if (obj.stat.gunHandling[i].gunName == "XM5") {
        user.playerStat.gunHandling.XM5 += obj.stat.gunHandling[i].shotFired * 0.05;
  ...
  if (squadMatch) {
    ...
    if (obj.typeOfEvent == constants.KILLED_BY_PLAYER_EVENT) {
      ...
      if (user) {
        ...
        if (obj.killType) await killsDataEventHandler(obj.killType, user, obj);
        await user.save();
      }
...
async function startSquadGameNew(io, obj, cb, socket) {
  ...
  if (squad) {
    ...
    if (squadMatch) {
      ...
      if (obj.subMode) squadMatch.subMode = obj.subMode;
      squad.team = squadMatch.members.length + 1;
      await squad.save();
...
async function connectToServer(io, obj, cb, socket) {
  ...
  if (server) {
    ...
    if (obj.port == 7777) {
      ...
      for (let i = 0; i < server.servers.length; i++) {
        if (
          server.servers[i].team <= server.servers[minIndex].team &&
          server.servers[i].port != 7777
        ) {
          minIndex = i;
        }
        if (server.servers[i].port == obj.port) {
