...
async function generateMap(squadMatch, io) {
  ...
  for (let i = 0; i < squadMatch.members.length; i++) {
    let b1 = Math.floor(
      Math.random() * (extractionJson.data[i].length - 0) + 0
    );
    let b2 = Math.floor(
      Math.random() * (extractionJson.data[i].length - 0) + 0
    );
  ...
  let a = Math.floor(
    Math.random() * (dronesJson.data[level].mapData.length - 0) + 0
  );
  ...
  let loots = {};
  let b = Math.floor(
    Math.random() * (lootsJson.data[level].mapData.length - 0) + 0
  );
  ...
  console.log(
    "generate map + " +
      JSON.stringify(squadMatch.drones) +
      "  " +
      JSON.stringify(squadMatch.extractions)
  );
